...
counters = {'approved_inwarded': 0, 'rejected_not_inwarded': 0, 'rejected_inwarded': 0}

# Define the conditional formatting function


def conditional_formatting_status(value):

    status = value['Status'].lower()
    accepted_rejected =  value['Accepted/Rejected'].lower()
    inwarded_not_inwarded = value['Inwarded/Not Inwarded'].lower()
...
filtered_data_styled = filtered_data.style.applymap(conditional_formatting_status).applymap(border).set_properties(**{'text-align': 'center'})
...
In which:<br><br>
 ...
 2).  {counters['rejected_not_inwarded']} JPINS were Rejected and Not Inwarded.(Highlighted in Red)<br><br>
 3).  {counters['rejected_inwarded']} JPINS were Rejected but Inwarded.(Highlighted in Yellow)<br><br>
"""

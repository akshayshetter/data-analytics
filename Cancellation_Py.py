...
# %%
query = """select distinct boi.order_item_id,boi.order_item_status,boi.order_id,boi.buyer_id as customer_id,ops.org_name as CustomerName,ps.jpin,ps.title as productTitle,boi.listing_id,ls.mrp,ls.selling_price,
boi.order_item_amount/boi.quantity as FinalPricePerUnit,boi.quantity as orderedQuantity,boi.created_units,boi.underprocessunitquantity,boi.ready_to_ship_units,boi.delivered_units,boi.cancelled_units,boi.seller_id,op.org_name as sellerName,boi.order_item_amount,boi.src_created_time as orderplacedtimestamp,boi.last_updated_by,boi.status_update_history,r.facilityid,date(pr.updated_promise_time) as promise_date,f.name as facility_name
...
--and boi.src_created_time>=current_date-30
and cat.category_name in ( 'Oil, Ghee and Vanaspati')
--and f.name='FC-Ahmedabad'
order by 25 desc
"""
...
sheet=gc.open('Cancellation Sheet')

wks_1=sheet.worksheet_by_title('Sheet1')

wks_1.clear(start='A',end='H')

...
#def conditional_formatting(value):
   # value = float(value.strip('%'))
    ...
    #else:
     #   return '' # Return empty string for non-percentage values
#%%
#def conditional_formatting(value):
   # try:
...
#percentage_columns = ['% Of Jpins Delivered vs Jpins Po Raised']

#%%
#for col in percentage_columns:
    #bkp_df2[col] = bkp_df2[col].apply(conditional_formatting)
#%%
# Reset index and set index starting from 1
...
#%%
temp3 = temp3.set_table_styles([
    {'selector': 'thead th', 'props': [('text-align', 'center')]}
])

temp4 = temp4.set_table_styles([
    {'selector': 'thead th', 'props': [('text-align', 'center')]}
])
# %%
print(temp3)
...
print(temp4)
# %%
#styled_df = temp3.style.applymap(border).set_properties(**{'text-align':'center'})
#%%
#styled_df2 = temp4.style.applymap(border).set_properties(**{'text-align':'center'})
#%%
...
bkp_new2['Date'] = pd.to_datetime(bkp_new2['Date'],format = '%d-%m-%Y')

yesterday2 = datetime.now()-timedelta(days=1)

yesterday_date2 =yesterday2.date()

yesterday_data3 = bkp_new2[bkp_new2['Date'].dt.strftime('%d-%m-%Y') == yesterday_date2.strftime('%d-%m-%Y')]

columns = ['City 2',	'JPIN',	'Title','System Suggested Cases',	'Avm Planned Cases',	'Po Raised Cases',	'Delivered Cases']

bkp_new3 = yesterday_data3[columns]
#%%
# Convert columns to numeric type
bkp_new3['System Suggested Cases'] = pd.to_numeric(bkp_new3['System Suggested Cases'], errors='coerce')
bkp_new3['Avm Planned Cases'] = pd.to_numeric(bkp_new3['Avm Planned Cases'], errors='coerce')

#%%
bkp_new3 = bkp_new3[(bkp_new3['Avm Planned Cases'] >= bkp_new3['System Suggested Cases']) |  (bkp_new3['Avm Planned Cases'] <= bkp_new3['System Suggested Cases'])]
...
styled_df2 = bkp_new3.style.set_properties(**{'text-align':'center'})
...
# vendor wise Po raised backup

wks_sh = gc.open_by_key('19LOa1geQijLD9AiBPAOGOvxsyQ_Mat6JFymUthI24AU')

wks_df = wks_sh.worksheet_by_title('Vendor Po backup')

wks_sh2 = wks_df.get_as_df(start='A1',end='O')

#%%
wks_sh2['Date'] = pd.to_datetime(wks_sh2['Date'],format = "mixed")
#%%
yesterday_data4 = wks_sh2[wks_sh2['Date'].dt.strftime('%d-%m-%Y') == yesterday_date2.strftime('%d-%m-%Y')]

col = ['City',	'JPIN','TITLE','Vendor Name',	'AVM Requested Cases',	'Po Raised Cases',	'Delivered Cases']

wks_sh4 = yesterday_data4[col]

#%%
print(wks_sh4)

#%%
styled_df3 = wks_sh4.style.set_properties(**{'text-align':'center'})

styled_bkp3 = styled_df3.set_table_styles([{'selector': 'thead th', 'props': [('text-align', 'center')]} #aligns header at center
,{'selector': 'td', 'props': [('text-align', 'center'), ('border', '1px solid black')]},]) # align text at center
#%%
styled_bkp3.to_excel('C:/Users/Dell/Documents/Oil Files/Daily_Vendor_Po_Summary.xlsx',index=False)
# %%
# mail script
...
receiver = 'akshay.shetter@jumbotail.com','srinivas.hn@jumbotail.com','suraj.singh@jumbotail.com','rajesh.unecha@jumbotail.com','vishal.kumar@jumbotail.com','krishna.sahugoud@jumbotail.com','tapan.khatua@jumbotail.com','sai.koushik@jumbotail.com','pratyush.priyadarshan@jumbotail.com','raghavendra.patil@jumbotail.com','divakaran.natarajan@jumbotail.com'


subject = 'Oil Planning Summary: {}'.format(yesterday_date.strftime('%d-%m-%Y'))
...
with open(f'C:/Users/Dell/Documents/Oil Files/Daily_Vendor_Po_Summary.xlsx','rb') as f:
    file_data = f.read()
    em.add_attachment(file_data, maintype="application", subtype="xlsx", filename=f'Daily_Po_Summary.xlsx')
